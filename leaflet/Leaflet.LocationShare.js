...
LS.Send = {}
LS.Send.Marker = {}
LS.Send.Popup = L.popup().setContent('<div><input id="sendText" type="text" style="border-color:#a7a7a7;border:solid;border-width:2px;border-radius:5px;height:30px;" size="30" onkeyup="L.LocShare.Send.UpdateMessage( this )" placeholder="输入您的寄语"/></div><div style="height:35px;"><button style="border-style:solid;border-radius:5px;border-color:#3d94f6;float:right;color:white;background-color:#3d94f6;height:35px;font-size:15px;line-height:3px;margin:5px;" onclick="copyPrompt()">善美之事，与众同乐</button></div></div>')
LS.Receive = {}
LS.Receive.Marker = {}
LS.Receive.Popup = L.popup()
var sendIcon = L.icon({
  iconUrl: "https://huweibiji.oss-cn-guangzhou.aliyuncs.com/ditu/leaflet/images/IconMapSend.png",
...
receiveIcon = L.icon({
  iconUrl: "https://huweibiji.oss-cn-guangzhou.aliyuncs.com/ditu/leaflet/images/IconMapReceive.png",
  iconSize:     [24, 24], // size of the icon
  iconAnchor:   [12, 12], // point of the icon which will correspond to marker's location
  popupAnchor:  [0, -10] // point from which the popup should open relative to the iconAnchor
})
...
L.Control.ShareLocation = L.Control.extend({
    ...
    onAdd: function () {
        ...
        userIcon.src = 'https://huweibiji.oss-cn-guangzhou.aliyuncs.com/ditu/leaflet/images/IconLocShare.png';
        userIcon.alt = '';
...
function showSuccessModal(shareLink) {
  ...
  content.innerHTML = '<strong style="color: red;">美好事物已为您打包到剪贴板</strong>';
