...
L.Control.Coordinates = L.Control.extend({
	...
	onAdd: function(map) {
		...
		ySpan.innerHTML = '【请输入坐标】 ' + options.labelTemplateLat.replace("{y}", "");
	...
	collapse: function() {
		if (!this._showsCoordinates) {
			...
			if (this._marker) {
				...
				label.innerHTML = '【当前定位】 ' + this._createCoordinateLabel(ll);
	...
	_update: function(evt) {
		...
		if (pos) {
			...
			// this._label.innerHTML = '点击此处自定义坐标，当前坐标 ' + this._createCoordinateLabel(pos);
			
			var title = document.createElement('span'); // 创建一个span元素
			title.innerHTML = '【点击此处自定义坐标】 ';
			title.style.color = 'red';
			title.style.fontWeight = 'bold';
			this._label.innerHTML = ''; // 清空原有内容
			this._label.appendChild(title); // 将title添加到label中
			this._label.innerHTML += this._createCoordinateLabel(pos); // 将坐标信息添加到label中
